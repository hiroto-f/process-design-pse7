"""Run four wall-cooled stages sized from 300 C inlet to 400 C outlet."""
...
OUTPUT_DIR = PACKAGE_DIR / "outputs" / "four_stage_300c_to_400c_cocurrent"
    ...
    for stage_index in range(base_case.stages.count):
        ...
        if outlet_temperature_k < TARGET_OUTLET_TEMPERATURE_K:
            raise RuntimeError(
                ...
                f"{TARGET_OUTLET_TEMPERATURE_K:.2f} K within {upper:.2f} m"
            )
        for _ in range(40):
