...
'Langmuirパラメータ
Dim qmax(Nall) As Double
Dim b(Nall) As Double

Dim C0(Nall) As Double '初期濃度 kmol/m3
Dim u0 As Double '空塔線速度
Dim Tt As Double '吸着塔操作温度

Dim qtz(Nall) As Double
Dim Ct(Nall, 2000) As Double 'C_t(i, z)
Dim qt(Nall, 2000) As Double 'q_t(i, z)
Dim countrec As Integer
Dim output_data_Cq(m, 4) As Double
Dim FlowOut(Nall) As Double '非吸着出口から出て行った量
Dim PurgeOut(Nall) As Double 'パージ出口から出て行った量
Dim decompOut(Nall) As Double '減圧時のメモ用
...
Sub Adsorption_1()
    Dim Ct_1(Nall, 2000) As Double 'C_t+1(i, z)
    Dim Ceq(Nall, 2000) As Double 'Ceq_t(i, z)
    Dim qt_1(Nall, 2000) As Double 'q_t+1(i, z)
    Dim Cin(Nall) '無次元初期濃度
    Dim qin(Nall) '無次元初期吸着量
    Dim t As Double
    Dim dt As Double
    Dim z As Double
    Dim dz As Double
    Dim flow(Nall) As Double 'モル流量 kmol/s
    ...
    Dim w As Double
    Dim f As Double
    Dim g(Nall) As Double
    Dim h(Nall) As Double
    ...
    Dim Kfav(Nall) As Double '総括物質移動係数 1/s
    ...
    '確認出力
    Dim output_data_Lang(Nall, 1) As Double
    For i = 0 To Nall
