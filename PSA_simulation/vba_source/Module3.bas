...
Sub From_HYSYS()
    ...
    'モル流量入力
    fi(0) = f(1).ComponentMolarFlow(4) * 3600 'H2 kmol/h
    fi(1) = f(1).ComponentMolarFlow(11) * 3600 'CH4
...
Sub Mix_Viscosity()
    ...
    For i = 0 To num
        ...
        output_data_mupsi(i, 1) = psi(i, 0)
        output_data_mupsi(i, 2) = psi(i, 1)
    Next
    ...
    For k = 0 To 1
        ...
        For i = 0 To num
            For j = 0 To num
                ...
                '2成分間での拡散係数
                Dai(i, j, k) = 0.0018583 * ((Tt * Tt * Tt * (1 / m(i) + 1 / m(j))) ^ 0.5) / (PP * 0.00986923 * sigmaD(i, j) * sigmaD(i, j) * omega(i))
                Dai(i, j, k) = Dai(i, j, k) / 10000 'cm2/s -> m2/s
            Next j
            ...
            For j = 0 To num
                If i <> j Then
                    Dam(i, k) = Dam(i, k) + x(j) / Dai(i, j, k)
                End If
            Next j
            Dam(i, k) = 1 / Dam(i, k) 'm2/s

            'Knudsen拡散係数
            Dkaa(i, k) = 3.067 * ra * ((Tt / (m(i) / 1000)) ^ 0.5) 'm2/s
            Dkai(i, k) = 3.067 * ri * ((Tt / (m(i) / 1000)) ^ 0.5) 'm2/s

            '粒子内総括拡散係数 m2/s
            Dea(i, k) = epsi * epsi * (1 + 3 * epsa) / (1 - epsa) / (1 / Dkai(i, k) + 1 / Dam(i, k)) + epsa * epsa / (1 / Dkaa(i, k) + 1 / Dam(i, k))
        Next i
    Next k
